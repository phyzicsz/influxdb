...
func NewConfig(test string) *Config {
	root, err := os.MkdirTemp("", "tests-influxdb-")
	if err != nil {
		panic(err)
	}

	fmt.Println("TEST:", test, "ROOTDIR:", root, "INDEXTYPE:", indexType)

	c := &Config{rootPath: root, Config: run.NewConfig()}
	c.BindAddress = "127.0.0.1:0"
	c.ReportingDisabled = true
	c.Coordinator.WriteTimeout = toml.Duration(90 * time.Second)
	...
	c.HTTPD.WriteTracing = verboseServerLogs
